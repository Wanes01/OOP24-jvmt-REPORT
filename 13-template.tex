...
\caption{Il diagramma mostra il pattern Strategy applicato a EndCondition e GemModifier, creati
dalle rispettive Factory Method e composti in RoundEffectImpl. RoundImpl può utilizzare
...
\begin{description}
	\item[\textbf{Problema}]: durante un round i giocatori possono uscire.
	...
	per ottenere con una sola chiamata il prossimo giocatore attivo ~\ref{img:playermanager}.

	\begin{figure}[H]   % H maiuscola
	\centering
	\includegraphics[width=0.4\textwidth]{img/playermanager}
	\caption{Il diagramma mostra il pattern Iterator applicato a RoundPlayerManager.}
...
\begin{description}
	...
	il controller ed imposta gli handler della visualizzazione senza preoccuparsi di come le pagine specifiche (le sue sottoclassi)
	...
	un comportamento di \texttt{ControllerAwarePage} comporterebbe probabilmente duplicazione di codice.
